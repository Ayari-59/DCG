"use client";

import { useState, useMemo } from "react";
import type { Fiche } from "@/lib/content/notions/dcg-ue11.generated";

const CATS = [
  { label: "Notion clé", cls: "key" },
  { label: "Formule / Calcul", cls: "form" },
  { label: "Méthode", cls: "meth" },
  { label: "Outil", cls: "tool" },
  { label: "Définition", cls: "def" },
] as const;

type CatCls = (typeof CATS)[number]["cls"];

const CAT_CLS: Record<string, CatCls> = Object.fromEntries(CATS.map((c) => [c.label, c.cls]));

const BORDER_TOP: Record<CatCls, string> = {
  key: "border-t-navy",
  form: "border-t-brand",
  meth: "border-t-teal-700",
  tool: "border-t-violet-600",
  def: "border-t-slate-500",
};

const BADGE_BG: Record<CatCls, string> = {
  key: "bg-navy",
  form: "bg-brand",
  meth: "bg-teal-700",
  tool: "bg-violet-600",
  def: "bg-slate-500",
};

const CHIP_ON: Record<CatCls, string> = {
  key: "!bg-navy !text-white !border-navy",
  form: "!bg-brand !text-white !border-brand",
  meth: "!bg-teal-700 !text-white !border-teal-700",
  tool: "!bg-violet-600 !text-white !border-violet-600",
  def: "!bg-slate-500 !text-white !border-slate-500",
};

function norm(s: string) {
  return s
    .normalize("NFD")
    .replace(/[̀-ͯ]/g, "")
    .toLowerCase();
}

export function BanqueNotions({
  fiches,
  chapitres,
  ue,
  diplome,
  matiere,
}: {
  fiches: Fiche[];
  chapitres: { num: string; titre: string }[];
  ue: string;
  diplome: string;
  matiere: string;
}) {
  const [recherche, setRecherche] = useState("");
  const [chapFiltre, setChapFiltre] = useState("");
  const [catFiltre, setCatFiltre] = useState("");

  const resultats = useMemo(() => {
    const terme = norm(recherche.trim());
    return fiches.filter((f) => {
      if (chapFiltre && f.ch !== chapFiltre) return false;
      if (catFiltre && f.cat !== catFiltre) return false;
      if (terme) {
        const texte = norm(
          [f.titre, f.definition, f.retenir, f.piege, ...f.points, ...f.formules].join(" "),
        );
        if (!texte.includes(terme)) return false;
      }
      return true;
    });
  }, [fiches, recherche, chapFiltre, catFiltre]);

  return (
    <>
      {/* Hero */}
      <header className="bg-gradient-to-br from-[#0f2547] via-navy-deep to-[#0e2a50] px-5 py-6 text-white sm:px-7">
        <div className="flex items-center gap-3.5">
          <svg
            viewBox="-8 -8 136 136"
            width="52"
            height="52"
            className="shrink-0 drop-shadow-[0_2px_4px_rgba(0,0,0,0.3)]"
            aria-hidden="true"
          >
            <mask id="e" maskUnits="userSpaceOnUse">
              <rect x="-8" y="-8" width="136" height="136" fill="#fff" />
              <path
                d="M6 104 C30 96 46 80 62 60 C78 40 90 28 108 18"
                fill="none"
                stroke="#000"
                strokeWidth="21"
                strokeLinecap="round"
              />
            </mask>
            <g mask="url(#e)">
              <circle cx="60" cy="60" r="40" fill="none" stroke="#f07e26" strokeWidth="14" />
              <circle cx="60" cy="60" r="23" fill="#16325c" />
            </g>
            <g fill="none" stroke="#fbb040" strokeWidth="9" strokeLinecap="round">
              <path d="M12 99 C33 91 47 78 62 59 C74 44 86 33 99 25" />
              <path d="M95.2 39.7 L104 20.7 L83.1 21.5" strokeLinejoin="miter" />
            </g>
          </svg>
          <div>
            <h1 className="m-0 font-serif text-[26px] font-bold text-white">Banque de notions</h1>
            <p className="mt-0.5 text-sm text-[#c9d6e8]">
              {diplome} · {ue} — {matiere} · {fiches.length} fiches de révision,{" "}
              {chapitres.length} chapitres
            </p>
          </div>
        </div>
      </header>

      {/* Barre d'outils */}
      <div className="sticky top-[57px] z-20 border-b border-line bg-white px-5 py-3.5 shadow-[0_2px_10px_rgba(16,32,60,0.05)] sm:px-7">
        <div className="flex flex-wrap items-center gap-3">
          <input
            type="search"
            value={recherche}
            onChange={(e) => setRecherche(e.target.value)}
            placeholder="🔎 Rechercher une notion, une formule, un mot-clé…"
            className="min-w-[220px] flex-[1_1_320px] rounded-[10px] border-2 border-line px-3.5 py-2.5 text-[15px] text-ink outline-none transition placeholder:text-slate-400 focus:border-brand"
          />
          <select
            value={chapFiltre}
            onChange={(e) => setChapFiltre(e.target.value)}
            className="max-w-[320px] rounded-[10px] border-2 border-line bg-white px-3 py-2.5 text-sm text-ink outline-none transition focus:border-brand"
          >
            <option value="">Tous les chapitres</option>
            {chapitres.map((c) => (
              <option key={c.num} value={c.num}>
                Ch. {c.num} — {c.titre}
              </option>
            ))}
          </select>
          <span className="ml-auto whitespace-nowrap text-[13px] font-semibold text-muted">
            {resultats.length} / {fiches.length} fiches
          </span>
        </div>

        {/* Chips catégories */}
        <div className="mt-2.5 flex flex-wrap gap-2">
          <button
            onClick={() => setCatFiltre("")}
            className={`rounded-full border-[1.5px] px-3.5 py-1.5 text-[13px] font-semibold transition ${
              !catFiltre
                ? "border-navy bg-navy text-white"
                : "border-line bg-white text-muted hover:border-slate-400"
            }`}
          >
            Toutes catégories
          </button>
          {CATS.map((c) => (
            <button
              key={c.label}
              onClick={() => setCatFiltre(catFiltre === c.label ? "" : c.label)}
              className={`rounded-full border-[1.5px] px-3.5 py-1.5 text-[13px] font-semibold transition ${
                catFiltre === c.label
                  ? CHIP_ON[c.cls]
                  : "border-line bg-white text-muted hover:border-slate-400"
              }`}
            >
              {c.label}
            </button>
          ))}
        </div>
      </div>

      {/* Grille */}
      <main className="px-5 py-6 pb-16 sm:px-7">
        {resultats.length === 0 ? (
          <p className="py-16 text-center text-base text-muted">
            Aucune fiche ne correspond à ta recherche.
          </p>
        ) : (
          <div className="grid grid-cols-[repeat(auto-fill,minmax(330px,1fr))] gap-4">
            {resultats.map((f, i) => (
              <Card key={i} fiche={f} />
            ))}
          </div>
        )}
      </main>

      {/* Footer */}
      <footer className="px-5 py-5 text-center text-xs text-muted sm:px-7">
        Objectif-DCG.fr · Banque de notions {ue} — {fiches.length} fiches · Astuce : combine la
        recherche, le filtre chapitre et les catégories.
      </footer>
    </>
  );
}

function Card({ fiche: f }: { fiche: Fiche }) {
  const cls = CAT_CLS[f.cat] ?? "def";

  return (
    <article
      className={`rounded-[14px] border border-line bg-white p-4 shadow-[0_1px_3px_rgba(16,32,60,0.05)] ${BORDER_TOP[cls]} border-t-[5px]`}
    >
      {/* En-tête */}
      <div className="mb-2 flex items-center gap-2">
        <span
          className={`rounded-full px-2.5 py-0.5 text-[11px] font-bold tracking-wide text-white ${BADGE_BG[cls]}`}
        >
          {f.cat}
        </span>
        <span className="ml-auto rounded-full bg-[#eef2f7] px-2.5 py-0.5 text-[11px] font-bold text-muted">
          Ch. {f.ch}
        </span>
      </div>

      {/* Titre */}
      <h3 className="mb-1.5 font-serif text-lg font-bold leading-tight text-navy">{f.titre}</h3>

      {/* Définition */}
      <p className="mb-2 text-sm leading-relaxed text-ink">{f.definition}</p>

      {/* SVG */}
      {f.svg && (
        <div
          className="mx-auto mb-2 max-w-[340px] rounded-lg border border-line bg-[#fbfcfe]"
          dangerouslySetInnerHTML={{ __html: f.svg }}
        />
      )}

      {/* Détails dépliables */}
      {(f.points.length > 0 || f.formules.length > 0 || f.methode.length > 0 || f.piege) && (
        <details className="group">
          <summary className="cursor-pointer list-none py-0.5 text-[13px] font-semibold text-brand [&::-webkit-details-marker]:hidden">
            <span className="group-open:hidden">▸ </span>
            <span className="hidden group-open:inline">▾ </span>
            Détails
          </summary>

          {f.points.length > 0 && (
            <ul className="my-2 space-y-1 pl-5 text-[13.5px] leading-relaxed text-ink marker:text-brand">
              {f.points.map((p, i) => (
                <li key={i}>{p}</li>
              ))}
            </ul>
          )}

          {f.formules.length > 0 && (
            <div className="my-2 flex flex-col gap-1.5">
              {f.formules.map((fm, i) => (
                <code
                  key={i}
                  className="rounded-lg border border-[#fadfbb] bg-[#fff7ec] px-2.5 py-1.5 font-mono text-[13.5px] text-[#9a5a00]"
                >
                  {fm}
                </code>
              ))}
            </div>
          )}

          {f.methode.length > 0 && (
            <ol className="my-2 list-decimal space-y-1 pl-5 text-[13.5px] leading-relaxed text-ink">
              {f.methode.map((m, i) => (
                <li key={i}>{m}</li>
              ))}
            </ol>
          )}

          {f.piege && (
            <p className="mt-2 rounded-md border-l-[3px] border-l-[#e05252] bg-[#fdecec] px-2.5 py-1.5 text-[13px] text-[#7a2020]">
              <b>⚠ Piège :</b> {f.piege}
            </p>
          )}
        </details>
      )}

      {/* À retenir */}
      {f.retenir && (
        <p className="mt-2.5 rounded-md border-l-[3px] border-l-teal-700 bg-[#eaf3ef] px-3 py-2 text-[13.5px] text-[#124f43]">
          <b>À retenir —</b> {f.retenir}
        </p>
      )}
    </article>
  );
}
