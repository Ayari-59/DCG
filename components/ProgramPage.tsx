          ...
          return (
            <section key={group.family + group.chapters[0].slug}>
              <div className="mb-4 flex items-center gap-3">
                ...
                <h2 className={`font-serif text-xl font-bold ${theme.text}`}>{group.family}</h2>
                <span className={`h-1 flex-1 rounded-full ${theme.bar} opacity-30`} />
              </div>
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="mt-3 flex flex-wrap items-center gap-2 text-xs">
                        <Pill className="bg-slate-100 text-slate-700">⏱ {c.durationMin} min</Pill>
                        {c.videos?.length ? (
                          <Pill className="bg-rose-100 text-rose-700">🎬 vidéo</Pill>
                        ) : null}
                        <Pill className="bg-emerald-100 text-emerald-700">
                          🃏 {c.flashcards.length}
                        </Pill>
                        <Pill className="bg-sky-100 text-sky-700">✅ {c.quiz.length}</Pill>
                      </span>
                    </span>
                    <span
...
function Pill({ children, className }: { children: React.ReactNode; className: string }) {
  return <span className={`rounded-full px-2.5 py-1 font-semibold ${className}`}>{children}</span>;
}
